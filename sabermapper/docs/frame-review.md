...
(20-30 fps over a few seconds) for the flash check. `game capture` hides notes, bombs, chains and arcs
continuously from the first probe frame to the end of the probe (they disappear once and come back once,
never blinking on the watched game window) and marks every frame rendered in that window
`notes_hidden: true`: nobody cuts notes during a capture, so uncut notes fly through the camera and fill up
to half of a single frame, which reads as a large-area flash although a player, who cuts them about 1 m
ahead, never sees it. The flash check therefore measures the scene (environment, lights, Vivify objects and
post-processing). A regular frame inside the probe window also has `notes_hidden: true`; the note corridor
check still applies to it, since it compares the configured note colours with the background. Frames outside
the window keep their notes. `game capture --probe-with-notes` keeps notes on the probe. A directory without `capture.json` is also read:
