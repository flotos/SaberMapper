...
def export_arrangement(arrangement: dict, audio: str | Path, cover: str | Path, output: str | Path) -> dict:
    ...
    if errors:
        raise ExportError("arrangement validation failed: " + "; ".join(str(x.get("message", x)) for x in errors))
    ...
    if beatmap.get("version") != "3.3.0" or not isinstance(beatmap.get("colorNotes"), list):
        raise ExportError("compiler did not return a v3.3.0 beatmap with colorNotes")
    if not beatmap["colorNotes"]:
        raise ExportError("compiled beatmap has no color notes")
    ...
    if latest_time >= audio_metadata["duration_seconds"] - 0.05:
        raise ExportError(f"last note at {latest_time:.3f}s exceeds playable audio duration {audio_metadata['duration_seconds']:.3f}s")
    ...
    info = {
        ...
        "_songAuthorName": song["artist"], "_levelAuthorName": str(arrangement.get("mapper") or "SaberMapper").strip(),
        ...
        "_difficultyBeatmapSets": [{"_beatmapCharacteristicName": "Standard", "_difficultyBeatmaps": [{
            ...
            "_noteJumpStartBeatOffset": difficulty.get("spawn_offset_beats", 0),
            "_beatmapColorSchemeIdx": 0, "_environmentNameIdx": 0,
        }]}],
    }
    report = {
        "format": "SaberMapper SM-029 export report 0.1", "beatmap_schema": "3.3.0",
        "info_schema": "2.1.0", "characteristic": "Standard", "difficulty": name,
        "audio_sha256": hashlib.sha256(audio_bytes).hexdigest(),
        "cover_sha256": hashlib.sha256(cover_bytes).hexdigest(),
        "arrangement_sha256": arrangement_revision(arrangement),
        "arrangement_hash_encoding": "canonical UTF-8 JSON; sorted keys, compact separators, no trailing newline",
        "color_note_count": len(beatmap["colorNotes"]),
        "audio_duration_seconds": audio_metadata["duration_seconds"],
        "last_note_seconds": latest_time,
    ...
    try:
        with destination.open("xb") as stream:
            with ZipFile(stream, "w") as archive:
                for filename, data in (("Info.dat", _json_bytes(info)), (f"{name}.dat", _json_bytes(beatmap)),
                                       ("song.ogg", audio_bytes), (cover_name, cover_bytes),
