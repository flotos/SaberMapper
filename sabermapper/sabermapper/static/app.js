...
function renderProject(audioChanged=false) {
  ...
  $('project-title').textContent=a.song.title;$('project-subtitle').textContent=`${a.song.artist} · ${a.difficulty.name}`;
...
async function saveArrangement(arrangement){state.project=await api(`/api/projects/${currentId()}/save`,{arrangement,revision:state.project.revision});renderProject();await refreshProjects();toast('Saved.');}
...
$('save-feedback').onclick=()=>run('Saving…',async()=>{await api(`/api/projects/${currentId()}/feedback`,{revision:state.project.revision,start_beat:state.selection[0],end_beat:state.selection[1],text:$('feedback-text').value});$('feedback-text').value='';state.project=await api(`/api/projects/${currentId()}`);renderProject();toast('Feedback saved.');});
...
$('toggle-lock').onclick=()=>run('Saving…',async()=>{const section=state.project.arrangement.sections.find(s=>s.id===state.section);state.project=await api(`/api/projects/${currentId()}/lock`,{section_id:state.section,locked:!section.locked,revision:state.project.revision});$('section-dialog').close();renderProject();toast(section.locked?'Unlocked.':'Locked.');});
$('update-timing').onclick=()=>run('Updating timing…',async()=>{state.project=await api(`/api/projects/${currentId()}/analyze`,{revision:state.project.revision,bpm:Number($('bpm').value),offset_seconds:Number($('offset').value)});renderProject();toast('Timing updated.');});
$('confirm-timing').onclick=()=>run('Saving…',async()=>{state.project=await api(`/api/projects/${currentId()}/review`,{revision:state.project.revision,timing_reviewed:!state.project.project.timing_reviewed});renderProject();toast('Saved.');});
$('restore-version').onclick=()=>run('Restoring…',async()=>{state.project=await api(`/api/projects/${currentId()}/restore`,{revision:state.project.revision,restore_revision:$('history').value});renderProject();toast('Version restored.');});
