...
def propose_rhythm(arrangement: dict, report: dict, *, start: float | None = None, end: float | None = None,
    ...
    two on a double) and the drafted arcs, ready to edit and save. ``held`` names held vocals (source seconds)
    the user asked for. ``listen`` is the listen run's section list: with the stems' rhythm it finds the song's
    recurring parts, and the draft declares a theme for each one touching the range (``themes``, kept beside
    any the arrangement already declares), so the placer plays a returning part like its first occurrence.
    """
    if not report:
