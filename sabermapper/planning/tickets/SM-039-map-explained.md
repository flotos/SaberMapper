...
- **Viewer** (`/viewer/`, opened by 3D preview and the section cards): ArcViewer (a compiled Unity build, left unmodified) in an iframe from the same origin, the map's explanation and current section on the left, and every section top to bottom on the right, folded to its name, start time and theme dots unless it is the current one (which unfolds to its summary, themes and progress); clicking any row jumps there. It reads ArcViewer's song clock from its window (`soundStartTime + (SongCtx.currentTime - lastPlayed) * playbackSpeed` while playing) and jumps by reloading ArcViewer with a new `t`.
