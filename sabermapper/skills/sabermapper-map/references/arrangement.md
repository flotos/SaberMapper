...
`themes` declares the song's recurring parts: an array of `{"id", "intent", "spans"[, "evidence"]}`. `id` is a unique lowercase identifier and `intent` names the recurring sound. `spans` lists the statement first, then one or more echoes, each `{"start_beat", "end_beat"}` in absolute beats; an echo may add `"mirror": true` and may be shorter than the statement (it then echoes the statement's opening). No beat belongs to two theme spans. `evidence` is free-form provenance, such as the listen sections or rhythm similarity. Placement gives each open echo note whose time matches a statement note (within 0.13 beat; a single on a single, a double on a double) the statement note's hand, cut and cell, mirrored when asked, unless a movement rule or a stored value says otherwise. Motifs repeat an identical figure; themes repeat a part whose rhythm follows each occurrence's own audio.
