...
| M4 asset forge | Done and verified in the game: Unity **2021.3.45f1** builds headless with the Hub Personal license (2021.3.16f1 cannot, `unity_headless_license_unsupported`); single-pass-instanced OpenXR bundle; a test show's grade and vignette Blits, ring prefab and particles render in FPFC with Vivify accepting the CRC. Tier-1 library (13 shaders, 9 mesh generators), lint, budgets. Tier 3 returns `generator_unavailable` (local backend chosen). | `assets/`, `forge*.py`, `docs/asset-forge.md` |
...
| M7 skill + first map | Skill `sabermapper-vivify` written. The whole chain (forge, show, export, capture) now runs; the first real vivified map is the next task. | `skills/sabermapper-vivify/` |
