...
def validate_arrangement(arrangement: dict) -> list[dict]:
    ...
    if not keys(arrangement, {"schema_version", "song", "difficulty", "motifs", "sections"}, "arrangement",
                optional={"tempo_events", "mapper", "lightshow", "themes", "style"} | ({"presentation"} if presented else set())):
        return findings
    ...
    if "themes" in arrangement:
        from .recurrence import validate_themes
        validate_themes(arrangement, add)

    if "style" in arrangement:
        from .style import validate_style
        validate_style(arrangement, add)
